...
- **c. 不要 replay，改独立源并修 bug**：独立源能谱**整体 ×1000 过热**(keV 被当 MeV)，
  外加 **×8 归一化**默认值错误。历史上曾用 `.sim.gz` IA INIT 分布做 debug 对照来定位单位 bug；
  这个对照不是最终 cross-code 证据，公开 handoff 已删除其脚本和产物。主线证据必须来自
  `sampled_source_authority` / isotope EventList 的独立源输运。
...
| B5 | **`source_parity` 假阳性** | `build_source_authority.py:246-267` 的 1M-sample 审计只检"采样器复现自己那张 CDF"，所以 CDF 单位错 1000× 也 PASS | 见 **§c-3**：需要独立于采样器自洽性的源语义审计；历史 `.sim.gz` 对照只作为 debug 旁证，不作为最终主线 |
...
> 修好 B5 的源语义审计，这类 bug 在 gate 处就会被拦下；历史 `.sim.gz` 对照不应升级成最终证据。
...
### c-3. 必加的 gate：源语义审计（堵住 B5）

`source_parity` 不能只验证采样器复现自己那张 CDF。至少要在源构建阶段保留以下
与输运无关的语义审计：
- 能量轴单位明确写入 artifact：输入列、输出 `energy_keV`、输出 `energy_MeV` 三者同时抽样记录；
- 抽样分位数(median、p10、p90)与源文件列分位数一致，禁止隐式 `×1000`；
- 方向(`dz`、`mu`)与出生位置(`|pos|`、xyz 分布)同样做独立分布摘要；
- 任一维偏离配置容差 → `BLOCKED_SOURCE_SEMANTICS`，不许进 transport。

`.sim.gz` IA INIT 分布可以作为历史 debug 对照来定位 TES/Cosima 实际使用的源语义，
但不能作为最终 cross-code 复现证据，也不应把 FLUKA 主线变成 replay/trace。
公开 handoff 因此删除了旧的 `validate_independent_source_against_sim.py` 与
`source_truth_validation/` 产物。
